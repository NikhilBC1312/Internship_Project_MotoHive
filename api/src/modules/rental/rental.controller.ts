...
export class RentalController {
  ...
  async getRentalById(@Param('id') id: string): Promise<RentalModel> {
    console.log('----- check12');
    return this.rentalService.findOne({ id: String(id) });
  }

  @Get('user/:id')
  @Roles(ROLES_ENUM.ADMIN)
  @UseGuards(JwtAuthGuard)
  async getRentalsByUser(@Param('id') id: string): Promise<RentalModel[]> {
    return this.rentalService.findAll({
  ...
  @Roles(ROLES_ENUM.ADMIN)
  @UseGuards(JwtAuthGuard)
  async createRental(@Body() createRentalDto: CreateRentalDto): Promise<RentalModel> {
    const { user_id, bike_id, ...rest } = createRentalDto;
    return this.rentalService.create({
      ...rest,
      User: {
        connect: { id: user_id.toString() },  // ✅ Convert to string
      },
      Bike: {
        connect: { id: bike_id.toString() },  // ✅ Convert to string
      },
    });
  }
  ...
  @Roles(ROLES_ENUM.ADMIN)
  @UseGuards(JwtAuthGuard)
  async updateRental(@Param('id') id: string, @Body() updateRentalDto: UpdateRentalDto): Promise<RentalModel> {
    const { user_id, bike_id, ...rest } = updateRentalDto;
    return this.rentalService.update({
      where: { id: String(id) },
      data: {
        ...rest,
        ...(user_id ? { User: { connect: { id: user_id.toString() } } } : {}),  // ✅ Fix
        ...(bike_id ? { Bike: { connect: { id: bike_id.toString() } } } : {}),  // ✅ Fix
      },
    });
  }
  ...
  async deleteRental(@Param('id') id: string): Promise<RentalModel> {
    return this.rentalService.delete({ id: String(id) });
  }
}
